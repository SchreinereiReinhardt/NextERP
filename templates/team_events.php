...
<div class="erp-head"><div><h1>Teamkalender</h1><p class="erp-sub">Der ausgewählte Nextcloud-Kalender ist die führende Quelle. Termine vom Handy werden beim Öffnen und über „Jetzt synchronisieren“ nach NextERP übernommen.</p></div><div class="erp-actions">
...
<div class="erp-integration-state is-connected erp-wide"><span>✓ Bidirektionaler Abgleich aktiv</span><strong><?php p($_['selectedCalendarName']); ?></strong><small>NextERP → Nextcloud sofort · Nextcloud/Handy → NextERP beim Öffnen oder manuellen Abgleich<?php if (!empty($_['lastCalendarSync'])): ?> · zuletzt <?php p(date('d.m.Y H:i', strtotime((string)$_['lastCalendarSync']))); ?><?php endif; ?></small></div>
...
<td><?php if (($r['sync_source'] ?? '') === 'nextcloud'): ?><span class="erp-badge">Handy / Nextcloud</span><?php elseif (!empty($r['calendar_object_uri'])): ?><span class="erp-badge">NextERP → Nextcloud</span><?php else: ?><span class="erp-muted">nur ERP</span><?php endif; ?></td>
</tr><?php endforeach; ?>
