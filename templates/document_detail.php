...
$recommendedProjectSupplierId = (int)($_['recommendedProjectSupplierId'] ?? 0);
?>
...
<form id="documentAssignForm" method="post" novalidate action="<?php p($url->linkToRoute('reinhardterp.document.assign', ['id' => $document['id']])); ?>">
    ...
    <div class="erp-form-grid">
        ...
        <div class="erp-span-2"><label>Lieferant</label><select id="documentSupplier" name="supplierId"><option value="">– kein Lieferant –</option><?php foreach ($_['suppliers'] as $supplier): ?><option value="<?php p($supplier['id']); ?>" <?=$selectedSupplier === (int)$supplier['id'] ? 'selected' : ''?>><?php p($supplier['name']); ?></option><?php endforeach; ?></select></div>
        <div class="erp-span-2 erp-supplier-link-box" id="supplierCockpitLinkBox"><div class="erp-section-head"><div><strong>Lieferanten & Wareneingang</strong><p class="erp-muted">Optional direkt mit einer Bestellung im Projekt verknüpfen. Damit erscheint die Auftragsbestätigung bzw. der Lieferschein sofort im Lieferanten-Cockpit.</p><?php if ($recommendedProjectSupplierId > 0): ?><p class="erp-smart-link-hint"><strong>✓ Smart-Vorschlag:</strong> Passender Lieferanten-/Bestellvorgang wurde vorausgewählt. Bitte kurz prüfen.</p><?php endif; ?></div></div><label>Lieferanten-/Bestellvorgang</label><select id="documentProjectSupplier" name="projectSupplierId"><option value="">– keine direkte Verknüpfung –</option><?php foreach (($_['projectSuppliers'] ?? []) as $sp): ?><option value="<?php p($sp['id']); ?>" data-project-id="<?php p($sp['project_id']); ?>" data-supplier-id="<?php p($sp['supplier_id']); ?>" <?=($recommendedProjectSupplierId === (int)$sp['id']) ? 'selected data-smart-recommended="1"' : ''?>><?php p(($sp['project_no'] ?? '').' · '.($sp['supplier_name'] ?? 'Lieferant').(!empty($sp['trade'])?' · '.$sp['trade']:'').(!empty($sp['purchase_no'])?' · Bestell-Nr. '.$sp['purchase_no']:'')); ?></option><?php endforeach; ?></select><div class="erp-form-grid erp-supplier-role-grid"><div><label>Im Lieferanten-Cockpit verwenden als</label><select id="supplierDocumentRole" name="supplierDocumentRole"><option value="">– auswählen –</option><option value="confirmation" <?=$selectedType==='order'?'selected':''?>>Auftragsbestätigung (AB)</option><option value="delivery" <?=$selectedType==='delivery_note'?'selected':''?>>Lieferschein / Wareneingang</option></select></div><div class="erp-supplier-link-hint"><span class="erp-status-pill">Projektbezug</span><small>Es werden nur Vorgänge des ausgewählten Projekts angeboten.</small></div></div></div>
