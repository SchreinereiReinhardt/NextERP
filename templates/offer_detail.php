...
<div class="erp-notice"><strong>Vorgang:</strong> Dieses Angebot wurde als Auftrag <a href="<?php p($url->linkToRoute('reinhardterp.business.orderDetail',['id'=>$lo['id']]));?>"><strong><?php p($lo['order_no']);?></strong></a> übernommen. Damit bleibt die Kette Angebot → Auftrag → Rechnung erhalten.</div>
<?php endif;?>
 ...
 <div class="erp-grid-2">
  ...
  <div class="erp-card"><h2>Summen</h2><div class="erp-kpi-line"><span>Netto</span><strong><?php p(number_format((float)$o['net_amount'],2,',','.').' €');?></strong></div><div class="erp-kpi-line"><span>USt. <?php p(number_format((float)$o['vat_rate'],2,',','.'));?> %</span><strong><?php p(number_format((float)$o['gross_amount']-(float)$o['net_amount'],2,',','.').' €');?></strong></div><div class="erp-kpi-line erp-total"><span>Brutto</span><strong><?php p(number_format((float)$o['gross_amount'],2,',','.').' €');?></strong></div>
   ...
   <form method="post" action="<?php p($url->linkToRoute('reinhardterp.business.createOrderFromOffer',['id'=>$o['id']]));?>"><input type="hidden" name="requesttoken" value="<?php p($_['requesttoken']);?>"><button class="button primary">Als Auftrag übernehmen</button></form>
  </div>
 </div>
</div></div>