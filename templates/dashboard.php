...
<?php if($can['offers']??false): ?><a class="erp-kpi-card" href="<?=p($url->linkToRoute('reinhardterp.business.offers'))?>"><span class="erp-kpi-icon"><i class="erp-ui-icon erp-icon-offer"></i></span><span>Offene Angebote</span><strong><?=p((int)($offerStats['count']??0))?></strong><small><?=p(number_format((float)($offerStats['gross']??0),2,',','.'))?> € brutto</small></a><?php endif; ?>
<?php if($can['invoices']??false): ?><a class="erp-kpi-card" href="<?=p($url->linkToRoute('reinhardterp.business.invoices'))?>"><span class="erp-kpi-icon"><i class="erp-ui-icon erp-icon-invoice"></i></span><span>Offene Rechnungen</span><strong><?=p((int)($invoiceStats['count']??0))?></strong><small><?=p(number_format((float)($invoiceStats['gross']??0),2,',','.'))?> € brutto</small></a><?php endif; ?>
