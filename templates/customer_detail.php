...
script('reinhardterp','customer_detail');
...
<header class="erp-customer-hero"><div><span class="erp-record-kicker">Kundenakte Pro</span><h1><?=p($customer->getCustomerNo().' · '.$customer->getName())?></h1><p><?=p($customer->getContactName()??'')?><?php if($customer->getPhone()): ?> · <a href="tel:<?=p($customer->getPhone())?>"><?=p($customer->getPhone())?></a><?php endif; ?><?php if($customer->getEmail()): ?> · <a href="mailto:<?=p($customer->getEmail())?>"><?=p($customer->getEmail())?></a><?php endif; ?></p></div><div class="erp-actions"><?php if($customerPath!==''): ?><a class="button primary" target="_blank" rel="noopener noreferrer" href="<?=p($folderUrl($customerPath))?>"><span class="erp-ui-icon erp-icon-folder"></span>Kundenordner</a><?php endif; ?><a class="button" href="<?=p($url->linkToRoute('reinhardterp.page.projectForm',['customerId'=>$customer->getId()]))?>">+ Neues Projekt</a><a class="button" href="<?=p($url->linkToRoute('reinhardterp.page.customerForm',['id'=>$customer->getId()]))?>">✏ Bearbeiten</a></div></header>

<nav class="erp-record-tabs erp-customer-tabs" id="erpCustomerTabs" aria-label="Kundenakte">
<button type="button" class="is-active" data-customer-tab="overview">Übersicht</button>
<button type="button" data-customer-tab="projects">Projekte</button>
<button type="button" data-customer-tab="documents">Dokumente</button>
<button type="button" data-customer-tab="reports">Rapporte</button>
<button type="button" data-customer-tab="contacts">Ansprechpartner</button>
<button type="button" data-customer-tab="reminders">Wiedervorlagen</button>
<button type="button" data-customer-tab="history">Historie</button>
<button type="button" data-customer-tab="more">Mehr</button>
</nav>

<div class="erp-customer-panel is-active" data-customer-panel="overview">
...
<section class="erp-customer-overview-grid">
<div class="erp-card"><div class="erp-section-head"><div><h2>Aktuelle Projekte</h2><p class="erp-muted">Direkter Blick auf die laufenden Arbeiten.</p></div><button type="button" class="button" data-open-customer-tab="projects">Alle Projekte</button></div>
<?php if(empty($projects)): ?><p class="erp-muted">Noch keine Projekte vorhanden.</p><?php else: ?><div class="erp-compact-links"><?php foreach(array_slice($projects,0,3) as $project): ?><a href="<?=p($url->linkToRoute('reinhardterp.page.projectDetail',['id'=>$project['id']]))?>"><span><strong><?=p($project['title'])?></strong><small><?=p($project['project_no'])?></small></span><b><?=p($project['status'])?></b></a><?php endforeach; ?></div><?php endif; ?>
</div>
<div class="erp-card"><div class="erp-section-head"><div><h2>Kontakt</h2><p class="erp-muted">Wichtigste Kundendaten auf einen Blick.</p></div><button type="button" class="button" data-open-customer-tab="more">Details</button></div><dl class="erp-overview-contact"><div><dt>Ansprechpartner</dt><dd><?=p($customer->getContactName()??'—')?></dd></div><div><dt>Telefon</dt><dd><?php if($customer->getPhone()):?><a href="tel:<?=p($customer->getPhone())?>"><?=p($customer->getPhone())?></a><?php else:?>—<?php endif;?></dd></div><div><dt>E-Mail</dt><dd><?php if($customer->getEmail()):?><a href="mailto:<?=p($customer->getEmail())?>"><?=p($customer->getEmail())?></a><?php else:?>—<?php endif;?></dd></div></dl></div>
<div class="erp-card"><div class="erp-section-head"><div><h2>Nächste Wiedervorlage</h2><p class="erp-muted">Rückruf oder nächster Schritt.</p></div><button type="button" class="button" data-open-customer-tab="reminders">Öffnen</button></div><?php $openReminders=array_values(array_filter($reminders,static fn($r)=>empty($r['is_done']))); ?><?php if(empty($openReminders)):?><p class="erp-muted">Keine offene Wiedervorlage.</p><?php else:$r=$openReminders[0];?><strong><?=p($r['title'])?></strong><p class="erp-muted">Fällig am <?=p(date('d.m.Y',strtotime($r['due_date'])))?></p><?php endif;?></div>
<div class="erp-card"><div class="erp-section-head"><div><h2>Letzte Dokumente</h2><p class="erp-muted">Zuletzt erfasste Dateien.</p></div><button type="button" class="button" data-open-customer-tab="documents">Dokumente</button></div><?php if(empty($documents)):?><p class="erp-muted">Noch keine Dokumente.</p><?php else:?><div class="erp-compact-links"><?php foreach(array_slice($documents,0,3) as $doc):?><a target="_blank" rel="noopener noreferrer" href="<?=p($fileUrl($doc['path']))?>"><span><strong><?=p($doc['name'])?></strong><small><?=p(date('d.m.Y',(int)$doc['mtime']))?></small></span></a><?php endforeach;?></div><?php endif;?></div>
</section>
</div>

<div class="erp-customer-layout">
<div class="erp-customer-main">
<section class="erp-card erp-wide erp-customer-panel" data-customer-panel="projects"><div class="erp-section-head"><div><h2>Projekte</h2><p class="erp-muted">Laufende und abgeschlossene Projekte dieses Kunden.</p></div></div>
...
<section class="erp-card erp-wide erp-document-center erp-customer-panel" data-customer-panel="documents"><div class="erp-section-head"><div><h2>Dokumentencenter</h2><p class="erp-muted">Letzte Dateien aus dem gesamten Kundenordner.</p></div></div>
...
<section class="erp-card erp-wide erp-customer-panel" data-customer-panel="history"><div class="erp-section-head"><div><h2>Kundenhistorie</h2><p class="erp-muted">Chronik aller Kunden-, Projekt-, Dokument- und Rapportaktivitäten.</p></div></div><?php print_unescaped($this->inc('_activity_timeline',['activities'=>$activities])); ?></section>
</div>

<aside class="erp-customer-sidebar">
<section class="erp-card erp-nextcloud-link-card erp-customer-panel" data-customer-panel="more"><div class="erp-section-head"><div><h2>Nextcloud Kontakte</h2><p class="erp-muted">Kundenstammdaten mit einem vorhandenen Nextcloud-Kontakt verbinden.</p></div></div>
...
<section class="erp-card erp-customer-panel" data-customer-panel="more"><h2>Kundendaten</h2><dl class="erp-info-list"><div><dt>Ansprechpartner</dt><dd><?=p($customer->getContactName()??'—')?></dd></div><div><dt>Telefon</dt><dd><?php if($customer->getPhone()):?><a href="tel:<?=p($customer->getPhone())?>"><?=p($customer->getPhone())?></a><?php else:?>—<?php endif;?></dd></div><div><dt>Mobil</dt><dd><?php if($customer->getMobile()):?><a href="tel:<?=p($customer->getMobile())?>">📱 <?=p($customer->getMobile())?></a><?php else:?>—<?php endif;?></dd></div><div><dt>E-Mail</dt><dd><?php if($customer->getEmail()):?><a href="mailto:<?=p($customer->getEmail())?>"><?=p($customer->getEmail())?></a><?php else:?>—<?php endif;?></dd></div><div><dt>Adresse</dt><dd><?php $addressLines=array_filter([$customer->getStreet(),trim((string)$customer->getPostalCode().' '.(string)$customer->getCity()),$customer->getCountry()]); ?><?= $addressLines ? nl2br(p(implode("\n",$addressLines))) : '—' ?></dd></div><div><dt>Rechnungs-E-Mail</dt><dd><?=p($customer->getInvoiceEmail()??'—')?></dd></div><div><dt>USt-IdNr.</dt><dd><?=p($customer->getVatId()??'—')?></dd></div><div><dt>Leitweg-ID</dt><dd><?=p($customer->getLeitwegId()??'—')?></dd></div><div><dt>Rechnungsformat</dt><dd><?=p(strtoupper($customer->getInvoiceFormat()?:'PDF'))?></dd></div></dl><?php if($customer->getNotes()):?><p class="erp-note-box"><?=nl2br(p($customer->getNotes()))?></p><?php endif;?></section>

<section class="erp-card erp-customer-panel" data-customer-panel="contacts"><div class="erp-section-head"><div><h2>Ansprechpartner</h2><p class="erp-muted">Zusätzliche Kontakte beim Kunden.</p></div></div>
...
<section class="erp-card erp-customer-panel" data-customer-panel="reminders"><div class="erp-section-head"><div><h2>Wiedervorlagen</h2><p class="erp-muted">Rückrufe und nächste Schritte.</p></div></div>
...
<section class="erp-card erp-customer-panel" data-customer-panel="reports"><h2>Rapporte</h2><?php if(empty($reports)):?><p class="erp-muted">Noch keine Rapporte.</p><?php else:?><div class="erp-compact-links"><?php foreach(array_slice($reports,0,8) as $report):?><a href="<?=p($url->linkToRoute('reinhardterp.module.reportDetail',['id'=>$report['id']]))?>"><span><strong><?=p($report['report_no'])?></strong><small><?=p($report['project_no'].' · '.$report['report_date'])?></small></span><b><?=p($report['status'])?></b></a><?php endforeach;?></div><?php endif;?></section>
</aside>
</div>
</main></div></div>
