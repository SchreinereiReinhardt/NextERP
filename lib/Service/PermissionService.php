...
final class PermissionService {
 ...
 public function roles():array{$qb=$this->db->getQueryBuilder();$qb->select('user_id','role','enabled','hourly_rate_id','individual_hourly_rate')->from('re_erp_user_roles');$out=[];foreach($qb->executeQuery()->fetchAll() as $r)$out[$r['user_id']]=['role'=>$r['role'],'enabled'=>(bool)$r['enabled'],'hourly_rate_id'=>$r['hourly_rate_id'],'individual_hourly_rate'=>$r['individual_hourly_rate']];return $out;}
}
