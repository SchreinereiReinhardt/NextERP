...
final class DocumentInboxService {
    ...
    public function assign(int $id, array $data): array {
        ...
        $allowedTypes = [
            ...
            'bank_statement', 'cash', 'tax', 'offer', 'order', 'report', 'drawing', 'other',
        ];
    ...
    public function rows(string $status = 'all', string $type = 'all', string $query = '', string $processing = 'all'): array {
        $this->syncInbox();
    ...
    private function buildName(array $doc, string $type, string $date, string $number, float $gross): string {
        $labels = [
            ...
            'report' => 'RAP', 'drawing' => 'PLAN', 'cash' => 'KASSE', 'tax' => 'STEUER', 'other' => 'DOK',
        ];
