...
final class Version000200Date20260802113000 extends SimpleMigrationStep {
 ...
 private function createReports(ISchemaWrapper $s): void {
  ...
  $t->setPrimaryKey(['id'], 're_erp_rep_pk'); $t->addUniqueIndex(['report_no'],'re_erp_report_no'); $t->addIndex(['project_id'],'re_erp_report_project');
 }
 private function createReportItems(ISchemaWrapper $s): void {
  ...
  $t->setPrimaryKey(['id'], 're_erp_rep_item_pk'); $t->addIndex(['report_id'],'re_erp_ri_report');
 }
 private function createReportHours(ISchemaWrapper $s): void {
  ...
  $t->addColumn('activity','string',['length'=>255,'notnull'=>false]); $t->setPrimaryKey(['id'], 're_erp_rep_hour_pk'); $t->addIndex(['report_id'],'re_erp_rh_report');
 }
 private function createReportFiles(ISchemaWrapper $s): void {
  ...
  $t->addColumn('created_at','datetime',['notnull'=>true]); $t->setPrimaryKey(['id'], 're_erp_rep_file_pk'); $t->addIndex(['report_id'],'re_erp_rf_report');
 }
 private function createMaterials(ISchemaWrapper $s): void {
  ...
  $t->addColumn('active','boolean',['default'=>1,'notnull'=>false]); $t->addColumn('created_at','datetime',['notnull'=>true]);
  $t->setPrimaryKey(['id'], 're_erp_mat_pk'); $t->addIndex(['name'],'re_erp_material_name');
 }
 private function createWorkdays(ISchemaWrapper $s): void {
  ...
  $t->setPrimaryKey(['id'], 're_erp_workday_pk'); $t->addIndex(['user_id','work_date'],'re_erp_workday_user_date');
 }
 private function createWorkdayEntries(ISchemaWrapper $s): void {
  ...
  $t->setPrimaryKey(['id'], 're_erp_work_ent_pk'); $t->addIndex(['workday_id'],'re_erp_wde_workday');
 }
 private function createTeamEvents(ISchemaWrapper $s): void {
  ...
  $t->setPrimaryKey(['id'], 're_erp_team_evt_pk'); $t->addIndex(['start_at'],'re_erp_event_start');
 }
 private function createProjectUsers(ISchemaWrapper $s): void {
  ...
  $t->setPrimaryKey(['id'], 're_erp_proj_usr_pk'); $t->addUniqueIndex(['project_id','user_id'],'re_erp_project_user_unique');
 }
}
