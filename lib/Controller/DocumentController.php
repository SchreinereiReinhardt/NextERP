...
final class DocumentController extends Controller {
 ...
 #[NoAdminRequired,NoCSRFRequired] public function index(string $status='all',string $type='all',string $q='',string $processing='all',string $year='',string $month='',?int $supplierId=null,?int $customerId=null,?int $projectId=null):TemplateResponse{$this->permissions->assert('documents');$this->documents->ensureStructure();return new TemplateResponse($this->appName,'document_inbox',['documents'=>$this->documents->rows($status,$type,$q,$processing,$year,$month,(int)$supplierId,(int)$customerId,(int)$projectId),'counts'=>$this->documents->counts(),'status'=>$status,'type'=>$type,'q'=>$q,'processing'=>$processing,'year'=>$year,'month'=>$month,'supplierId'=>(int)$supplierId,'customerId'=>(int)$customerId,'projectId'=>(int)$projectId,'suppliers'=>$this->documents->lookupRows('re_erp_suppliers','name'),'customers'=>$this->documents->lookupRows('re_erp_customers','name'),'projects'=>$this->documents->lookupRows('re_erp_projects','project_no'),'scanInfo'=>$this->documents->scanInfo(),'rules'=>$this->rules->all(),'message'=>(string)$this->request->getParam('message',''),'error'=>(string)$this->request->getParam('error','')]);}
