...
final class ProjectController extends Controller {
 ...
 #[NoAdminRequired] public function restore(int $id):RedirectResponse{$this->permissions->assert('projects');$p=$this->mapper->find($id);$p->setIsArchived(false);$p->setUpdatedAt(new \DateTime());$this->mapper->update($p);$this->activities->record('project',$id,'restored','Projekt wiederhergestellt',$p->getProjectNo().' · '.$p->getTitle(),$p->getCustomerId(),$id);return new RedirectResponse($this->url->linkToRoute('reinhardterp.page.projects',['view'=>'archive']));}
